...
cf push $NEW_APP -f ./mspsds-web/manifest.yml -d $DOMAIN --hostname $NEW_HOSTNAME --no-start
cf set-env $NEW_APP MSPSDS_HOST "https://$NEW_HOSTNAME.$DOMAIN"
cf start $NEW_APP
...
if [[ $SPACE == "prod" ]]; then
    cf unbind-service $NEW_APP mspsds-auth-env
fi

# Map the live hostname to the new app
cf set-env $NEW_APP MSPSDS_HOST "https://$HOSTNAME.$DOMAIN"
cf restart $NEW_APP
