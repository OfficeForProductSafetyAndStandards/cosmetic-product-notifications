...
RSpec.describe Types::TriggerQuestionQueries, type: :request do
  ...
  describe "trigger_questions query" do
    let!(:latest_trigger_question) { create(:trigger_question, created_at: "2024-08-10", updated_at: "2024-08-15", component: create(:component)) }
    let!(:middle_trigger_question) { create(:trigger_question, created_at: "2024-08-12", updated_at: "2024-08-16", component: create(:component)) }
    let!(:older_trigger_question) { create(:trigger_question, created_at: "2024-08-01", updated_at: "2024-08-05", component: create(:component)) }
    ...
    it "returns the total number of TriggerQuestions" do
      response_json = query_trigger_questions(first: 10) # Query without filters
      expect(response_json.dig("data", "trigger_questions", "edges").size).to eq(3) # Expecting 3
    end

    it "returns the associated question for the first trigger question" do
      response_json = query_trigger_questions(first: 10)
      expect(response_json.dig("data", "trigger_questions", "edges").first["node"]["question"]).to eq(latest_trigger_question.question)
    end
