...
RSpec.describe CpnpNotificationExporter do
  ...
  let(:notification_file_basic) { create_cpnp_parser }
  let(:notification_file_shades_import) { create_cpnp_parser("testWithShadesAndImport.zip") }
  let(:notification_file_multi_component_exact_formula) { create_cpnp_parser("testMultiComponentExactFormula.zip") }
  let(:notification_file_manual_ranges_trigger_rules) { create_cpnp_parser("testManualRangesTriggerRules.zip") }
  let(:notification_file_nano_materials_cmr) { create_cpnp_parser("testWithNanomaterialsAndCmrs.zip") }
  let(:notification_file_formulation_required) { create_cpnp_parser("testFormulationRequiredExportFile.zip") }
  let(:notification_file_different_language) { create_cpnp_parser("testDifferentLanguage.zip") }
