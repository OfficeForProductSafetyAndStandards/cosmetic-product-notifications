...
def notify_product(notification_type, product_category, _product_formulation)
  expected_h1("Internal reference")
  select_radio("No")
  click_button "Continue"
  if notification_type == "post brexit"
  ...
  if notification_type == "post brexit"
    ...
    page.attach_file("spec/cucumber/face_covering.jpg")
    click_button "Continue"
  end
...
def validate_check_your_answer_page(product_name, product_category, product_formulation)
  ...
  page.all("tr").each do |tr|
    next unless tr.has_css?("td.id", text: "product-table")

    expect(tr).to have_css("td.name", text: product_name)
    expect(tr).to have_css("td.Category of product", text: product_category)
    expect(tr).to have_css("td.Formulation given as", text: product_formulation)
    # other expects here...
  end
end
