...
RSpec.feature "Setting up app authentication", :with_2fa, :with_2fa_app, :with_stubbed_notify, type: :feature do
  ...
  feature "user has no app authentication configured" do
    ...
    scenario "user sets up app authentication" do
      skip("Reason: moving to GOV.UK onelogin")

      # User visits their account
      visit "/sign-in"
      fill_in_credentials
      ...
      # User needs to go through their current 2FA method (SMS) prior to changing the 2FA methods.
      expect_to_be_on_secondary_authentication_sms_page
      expect_user_to_have_received_sms_code(otp_code)
      complete_secondary_authentication_sms_with(otp_code)
      ...
      # Confirm App is now set as the primary 2FA method
      force_2fa_for_app_setup
      click_on "Change authenticator app"

      # Since SMS is still available for existing users, they can choose to change their authenticator app.
      # However, they should not be presented with a method selection page when changing the authenticator app.
      expect(page).to have_css("h1", text: "Change your authenticator app")
    end
  end

  feature "user has previously set up app authentication" do
    let(:user) do
    ...
    scenario "user updates app authentication" do
      ...
      # User needs to go through their current app access code prior to the update
      expect_to_be_on_secondary_authentication_app_page
      complete_secondary_authentication_app
  ...
  feature "user has previously set up both text and app authentication" do
    let(:user) do
    ...
    scenario "user updates app authentication" do
      ...
      # User selects the app for secondary authentication
