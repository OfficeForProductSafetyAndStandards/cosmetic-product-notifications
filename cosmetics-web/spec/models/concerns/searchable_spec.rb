...
RSpec.describe Searchable, type: :model do
  ...
  describe ".import_to_opensearch" do
    ...
    after do
      dummy_class.__elasticsearch__.client.indices.delete(
        index: "#{dummy_class.index_name},#{new_index_name},#{previous_index_name}",
    ...
    context "with forced creation" do
      ...
      it "imports the records to the new index" do
        travel_to execution_time do
          import
          expect(dummy_class).to have_received(:import).with(index: new_index_name, scope: "opensearch")
        end
      end
    end
    ...
    context "without forced creation" do
      ...
      context "when there was a previous index" do
        ...
        it "imports the records to the current index" do
          travel_to execution_time do
            import
            expect(dummy_class).to have_received(:import).with(index: previous_index_name, scope: "opensearch")
          end
        end
      end
  ...
  describe ".create_new_index_with_alias!" do
    ...
    after do
      travel_back
      dummy_class.__elasticsearch__.client.indices.delete(index: expected_index, ignore_unavailable: true)
    end
  ...
  describe ".alias_index!" do
    after do
      dummy_class.__elasticsearch__.client.indices.delete(index: "dummies_version", ignore_unavailable: true)
    end
