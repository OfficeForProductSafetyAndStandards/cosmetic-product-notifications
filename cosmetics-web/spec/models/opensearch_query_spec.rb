...
RSpec.describe OpensearchQuery, type: :model do
  ...
  context "when search term is provided and category filter is empty" do
    it_behaves_like "correct query" do
      let(:expected_es_query) do
        { query: { bool: { filter: [], must: { multi_match: { fuzziness: 0, query: "Foo bar", operator: "AND" } } } }, sort: %w[_score] }
      end
    end
  end
  ...
  context "when search term is provided and category filter is not empty" do
    it_behaves_like "correct query" do
      ...
      let(:expected_es_query) do
        { query: { bool: { filter: [{ nested: { path: "components", query: { bool: { should: [{ term: { "components.display_root_category": "Bar baz" } }] } } } }], must: { multi_match: { fuzziness: 0, query: "Foo bar", operator: "AND" } } } }, sort: %w[_score] }
      end
    end
  end
  ...
  context "when dates are provided" do
    it_behaves_like "correct query" do
      ...
      let(:expected_es_query) do
        { query: { bool: { filter: [{ nested: { path: "components", query: { bool: { should: [{ term: { "components.display_root_category": "Bar baz" } }] } } } }, { range: { notification_complete_at: { gte: Date.new(2021, 6, 6), lte: Date.new(2021, 6, 16) } } }], must: { multi_match: { fuzziness: 0, query: "Foo bar", operator: "AND" } } } }, sort: %w[_score] }
      end
    end
  end
  ...
  context "when using fuzzy finder" do
    it_behaves_like "correct query" do
      ...
      let(:expected_es_query) do
        { query: { bool: { filter: [], must: { multi_match: { fuzziness: "AUTO", query: "Foo bar", operator: "AND" } } } }, sort: %w[_score] }
      end
    end
  end
end
