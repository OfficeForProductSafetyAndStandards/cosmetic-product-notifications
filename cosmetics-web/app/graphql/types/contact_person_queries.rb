module Types
  module ContactPersonQueries
    ...
    included do
      ...
      # Add cursor-based pagination for contact_persons with filtering by created_at and updated_at
      field :contact_persons, ContactPersonType.connection_type, null: false, camelize: false, description: <<~DESC do
        ...
        ```
        query {
          contact_persons(created_after: "2024-08-15T13:00:00Z", updated_after: "2024-08-15T13:00:00Z", first: 10) {
            edges {
              node {
                id
                name
      ...
      # Query for retrieving the total number of contact persons available
    ...
    # Method to return contact persons with optional filters for created_at and updated_at, along with pagination support
    def contact_persons(created_after: nil, updated_after: nil, first: nil, last: nil, after: nil, before: nil)
      max_limit = 100
    ...
    def contact_persons_created_after(created_after:)
      filtered_by_date(field: "created_at", date: created_after)
    end

    def contact_persons_updated_after(updated_after:)
    ...
    private
      ...
      def filtered_by_date(field:, date:)
        ContactPerson.where("#{field} >= ?", date)
      rescue ArgumentError
