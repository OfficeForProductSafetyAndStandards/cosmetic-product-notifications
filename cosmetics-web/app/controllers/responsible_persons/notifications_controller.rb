...
class ResponsiblePersons::NotificationsController < SubmitApplicationController
  ...
  def destroy
    ...
    redirect_to responsible_person_notifications_path(@responsible_person, anchor: "incomplete"), confirmation: "#{@notification.product_name} notification deleted"
  end
...
private
  ...
  def get_unfinished_notifications(page_size)
    @responsible_person.notifications
      .where.not(state: :notification_complete)
