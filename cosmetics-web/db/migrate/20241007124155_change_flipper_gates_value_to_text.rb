...
class ChangeFlipperGatesValueToText < ActiveRecord::Migration[7.1]
  disable_ddl_transaction! # Disable DDL transaction to allow concurrent index operations
  ...
  def down
    ...
    remove_index :flipper_gates, %i[feature_key key value], algorithm: :concurrently if index_exists?(:flipper_gates, %i[feature_key key value])

    add_index :flipper_gates, %i[feature_key key value], unique: true if index_exists?(:flipper_gates, %i[feature_key key value])
  end
end
