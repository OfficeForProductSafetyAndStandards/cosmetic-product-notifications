module GdprHelper
  def can_be_displayed?(document, parent)
    document_creator = User.find_by(id: document.metadata[:created_by])
    child_is_visible = UserSource.new(user: document_creator).user_has_gdpr_access? || !document.metadata[:has_consumer_info]
